...
<br><br><br>
## 주의사항
* 테스트 계정으로 카드 결제시에는 부분취소가 되지 않습니다.
<br><br><br>
## 가맹점 인증 
...
PCD_RST_URL | 결제(요청)결과 RETURN URL | O | - 팝업방식은 상대경로, 다이렉트 방식은 절대경로로 설정  
PCD_PAYER_ID | 결제결과 리턴 빌링키 | - | 해당 키를 통해 결제요청 (PCD_SIMPLE_FLAG가 Y일 때 필수. 카드등록 또는 결제 후 재결제 시 요청)
